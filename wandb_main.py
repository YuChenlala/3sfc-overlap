...
wandb.init(
    ...
    config={
        ...
        'n_client_epoch': args.n_client_epoch,
...
for c_round in range(1, args.n_epoch + 1):
    ...
    if args.method == "fedavg":
        ...
        for client in participating_clients:
            ...
            best_inputs, best_labels, scale_factor, _cos = client.compute_synthetic_sample_change(args.ours_n_sample, n_classes,args.power)
            scale_factors.append(scale_factor)
            synthetics.append((best_inputs, best_labels))

            cos.append(abs(_cos.item()))
        server.aggregate_synthetic_gradients_change(synthetics, scale_factors, [client.dW for client in participating_clients])
