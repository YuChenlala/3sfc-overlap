...
def get_dataset(args):
    if args.dataset == 'mnist':
        data = datasets.MNIST(root=".", download=True, transform=transforms.Compose([transforms.ToTensor()]))
        n_classes = 10
    elif args.dataset == 'fmnist':
        data = datasets.FashionMNIST(root=".", download=True, transform=transforms.Compose([transforms.ToTensor()]))
        n_classes = 10
    elif args.dataset == 'emnist':
        data = datasets.EMNIST(root=".", split="byclass", download=True, transform=transforms.Compose([transforms.ToTensor()]))
        n_classes = 62
    elif args.dataset == 'cifar10':
        data = datasets.CIFAR10(root=".", download=True, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        ]))
        n_classes = 10
    elif args.dataset == 'cifar100':
        data = datasets.CIFAR100(root=".", download=True, transform=transforms.Compose([
            transforms.ToTensor(),
    ...
    client_data = [CustomSubset(data, idcs, transforms) for idcs in client_idcs]
    test_data = CustomSubset(data, test_idcs, transforms)
